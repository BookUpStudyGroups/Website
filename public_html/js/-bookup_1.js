	...
	if($("#search_group_body #map").length){
        ...
        function init(){
			...
			$("#available-groups-table tr").each(function(index){
				...
				var iconIndex = $("#user-classes li."+class_id).index() + 1;
				...
				marker.addListener('click', function() {
					...
					var class_name = row.find('.class_id').text();
					var t_name = row.find('.topicname').text();
					var on_date = row.find('.on_date').text();
					var time = row.find('.start_time').text() + ' - ' + row.find('.start_time').text();
			...
			$("#user-classes input[type=checkbox]").on('change', function(e){
				...
				$("#available-groups-table tr").each(function(index){
					...
					if( cur_classes.indexOf( row_class_id ) == -1 )
						return;
