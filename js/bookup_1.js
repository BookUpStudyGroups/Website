    ...
    success: function(invites) {
        ...
        var getInvites = new RSVP.Promise(function(fulfill) {
            ...
            for(i=0; i<invites.length; i++){
                var invites= invites[i].get("group");
                groupsWhoInvited.push(invites);
            }
            console.log("in promise");
        ...
        getInvites.then(function(groupsWhoInvited){
            ...
            var groupObj= Parse.Object.extend("studyGroup");
                    

            var classesQuery = new Parse.Query(groupObj);
            classesQuery.equalTo("studyGroup", groupObj);
            classesQuery.find({
                success: function(groups) {

                    // //remove group from invite array if wasn't invited to
                    for(w=0; w<groupsWhoInvited.length; w++){
                        var index1= groups.indexOf(groupsWhoInvited[w]);
                         if(index1!= -1){
                            console.log(groups);
                            var index = groups;
                        }
